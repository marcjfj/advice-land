...
import { loadModel, isChildOf } from './utils';
...
// DICE ANIMATION
let numberOfRolls = 0;
export const diceAnimation = (dice: THREE.Group) => {
  ...
  const xQuaternionKF = new QuaternionKeyframeTrack(
    '.quaternion',
    [0, 1],
    [
      qInitial.x,
  ...
  const yQuaternionKF = new QuaternionKeyframeTrack(
    '.quaternion',
    [0, 1],
    [
      q2Initial.x,
  ...
  const positionKF = new VectorKeyframeTrack(
    '.position',
    [0, 0.5, 1],
    [0, 0.475, 0, 0, 2, 0, 0, 0.575, 0]
  );
  const clip = new AnimationClip('Action', 1, [
    xQuaternionKF,
    yQuaternionKF,
    positionKF,
  ]);
...
export const intersectionWatcher = (query: string, intersected: any[]) => {
  if (intersected.length > 0) {
    if (isChildOf(query, intersected[0].object)) {
